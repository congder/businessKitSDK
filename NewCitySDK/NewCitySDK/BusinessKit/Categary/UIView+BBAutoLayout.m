//
//  UIView+BBAutoLayout.m
//
//  Created by banbao on 15/10/23.
//  Copyright (c) 2015年 banbao. All rights reserved.
//
...
@implementation BBAutoLayoutModel
...
#if defined DEBUG && defined BBAutoLayoutIssueLog // 调试状态下 约束问题提醒
        
        [weakSelf.needsAutoResizeView addOwnAutoLayoutModel:weakSelf withKey:key];
        
#endif
        
        BBAutoLayoutModel *newModel = [BBAutoLayoutModel new];
...
@implementation UIView (BBAutoLayout)
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method bb_autolayout = class_getInstanceMethod(self, @selector(bb_autolayout));
        method_exchangeImplementations(layoutSubviews, bb_autolayout);
    });
}

#pragma mark - properties

- (void)addLayoutModelWithAttributeHeightOrWidth:(BBAutoLayoutModel *)model
{

    __block long index = 0;
    __block BOOL found = NO;
    [self.autoLayoutModelsArray enumerateObjectsUsingBlock:^(BBAutoLayoutModel *obj, NSUInteger idx, BOOL *stop) {
...
- (BBAutoLayoutModel *)bb_layout
{
    BBAutoLayoutModel *model = [BBAutoLayoutModel new];
...
- (void)bb_autolayout
{
    ...
    if (self.autoLayoutModelsArray.count) {
        [self.autoLayoutModelsArray enumerateObjectsUsingBlock:^(BBAutoLayoutModel *model, NSUInteger idx, BOOL *stop) {
            [self bb_resizeWithModel:model];
        }];
    }
...
- (void)bb_resizeWithModel:(BBAutoLayoutModel *)model
{
...
- (void)addAutoLayoutModel:(BBAutoLayoutModel *)model
{
    [self.autoLayoutModelsArray addObject:model];
}
...
#define BBAutoLayoutAttributeLeftKey        @"水平方向约束（left）"
#define BBAutoLayoutAttributeRightKey       @"水平方向约束（right）"
#define BBAutoLayoutAttributeTopKey         @"垂直方向约束（top）"
#define BBAutoLayoutAttributeBottomKey      @"垂直方向约束（bottom）"
#define BBAutoLayoutAttributeWidthKey       @"宽度约束（width）"
#define BBAutoLayoutAttributeHeightKey      @"高度约束（height）"

- (void)addOwnAutoLayoutModel:(BBAutoLayoutModel *)model withKey:(NSString *)key
{
    ...
    if ([key rangeOfString:@"left" options:NSCaseInsensitiveSearch].length || [key rangeOfString:@"centerX" options:NSCaseInsensitiveSearch].length) {
        modelDict = @{BBAutoLayoutAttributeLeftKey : model};
    } else if ([key rangeOfString:@"right" options:NSCaseInsensitiveSearch].length) {
        modelDict = @{BBAutoLayoutAttributeRightKey : model};
...
- (void)checkAutoLayoutIntegrality
{
    if ([self ownAutoLayoutModelsArray].count && [self ownAutoLayoutModelsArray].count < 4) {
        ...
        [[self ownAutoLayoutModelsArray] enumerateObjectsUsingBlock:^(NSDictionary *dict, NSUInteger idx, BOOL *stop) {
            ...
            BBAutoLayoutModel *model = [dict.allValues firstObject];
...
@implementation UIView (BBChangeFrameCore)
